...
foreach($nameArray as $nameData) {
	...
	while($loopThrough == 1) {
		...
		$query = urlencode("affil(covenant university) AND authfirst(" . $firstName . ") AND authlast(" . $lastName . ")");
		...
		// Otherwise, proceed with returned results
			...
			if(is_null($totalResults)) {
				...
				if($totalResults == 0) {
					print "<label class ='my-error'> \tNo results found.\n</label >";
			...
			foreach($authors as $key => $authorInfo) {
				...
				if(isset($authorInfo['error'])) {// The isset added by me To remove undefined index error
					$thisError = $authorInfo['error'];
					if($thisError !== "Result set was empty") {
						print "<label class ='my-error'> Error message: " . $thisError . "</label> \n";
					}
				...
				// Otherwise, proceed with author entry					
                    ...
                    //var_dump($authorInfo);                    
				    $applicantscopid  =  substr($authorInfo['eid'],7);
                    include('get-author-publications.php');
                    include('../promotion_validation.php');
                    //var_dump($applicantscopid);                    
					//var_dump($authorInfo);
